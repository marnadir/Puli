 ...
 */
public class AxiomDerivabilityChecker<C, I extends Inference<? extends C>>
		implements DerivabilityCheckerWithBlocking<C, I>, Proof<I> {

	// logger for this class
	private static final Logger LOGGER_ = LoggerFactory
			.getLogger(AxiomDerivabilityChecker.class);
	...
	public AxiomDerivabilityChecker(Proof<? extends I> proof) {
	...
	private void toCheck(C conclusion,I inf) {
		if (goals_.add(conclusion)) {
			...
			if(inf !=null) {
				if (blocked_.contains(conclusion) && inf.getPremises().size()==0) {
	...
	private Queue<I> getRemainingInferences(C conclusion) {
		...
		I temp=null;
		for(I inf:result) {
			if(inf.getConclusion().equals(blocked_.toArray()[0]) && inf.getPremises().size()==0) {
				temp=inf;
			}
		}
		if(temp!=null) {
